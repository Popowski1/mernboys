...
if (!correctPw) { throw new AuthenticationError('incorrect pass');}
const token = signToken(user);
return { token, user };
},
...
saveBook: async (parent, { bookId }, context) => {
    if (context.user) {
        const updatedUser = await User.findByIdAndUpdate( {_id: context.user._id} , { $pull: {savedBooks: {newBook}}},
                                                            {new: true});
                                                            return updatedUser;}
throw new AuthenticationError('log in please');
    ...
    removeBook: async (parent, { bookId }, context) => {
        if (context.user) {
            const updatedUser = await User.findByIdAndUpdate( {_id: context.user._id} , { $pull: {savedBooks: {bookId}}},
                                                                {new: true});
                                                                return updatedUser;}
